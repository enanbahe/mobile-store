pipeline {
  ...
  stages {
    stage('build-env') {
      ...
      steps {
        sh 'if (( $(export AWS_DEFAULT_REGION=$AWS_REGION && aws ecr describe-repositories | grep $REPO_NAME | wc -l) == 0 )); then aws ecr create-repository --repository-name $REPO_NAME; fi'
