pipeline {
  ...
  stages {
    stage('build-env') {
      ...
      steps {
        sh 'aws --version'
        sh 'export AWS_DEFAULT_REGION=$AWS_REGION && aws ecr describe-repositories && aws ecr create-repository --repository-name $REPO_NAME && $(aws ecr get-login --no-include-email --region $AWS_REGION)'
